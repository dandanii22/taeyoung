...
const sub1 = () => {
  ...
  $historyYear.forEach((hisYear, index) => {
    hisYear.addEventListener("click", (e) => {
      $historyBg.style.backgroundImage = `url(images/sub1/con2_bg${
        index + 1
      }.png)`;
  ...
  function fn(n) {
    gallery.slice(4 * n - 4, 4 * n).forEach((item, index) => {
      item.addEventListener("click", (e) => {
        $bImgs[n - 1].setAttribute(
          "src",
          shape2_bImg.slice(4 * n - 4, 4 * n)[index]
        );
      });
    });
  }
...
const login = () => {
  let $idinput = get("#userid");
  let $pwinput = get("#pw1");
  let $loginButton = get(".loginbtn");
...
const join = () => {
  let $idinput = get("#userid");
  let $nameinput = get("#username");
  let $pw1input = get("#pw1");
  let $pw2input = get("#pw2");
  let $chkinput = get("#chk1");
  let $joinButton = get(".joinbtn");

  $idinput.addEventListener("keyup", validate);
  $nameinput.addEventListener("keyup", validate);
  $pw1input.addEventListener("keyup", validate);
  $pw2input.addEventListener("keyup", validate);
  $chkinput.addEventListener("keyup", validate);
  $joinButton.addEventListener("click", errorEvent);
  ...
  function validate() {
    if (
      ...
      $joinButton.classList.remove("on");
    } else {
      $joinButton.disabled = false;
      $joinButton.classList.add("on");
    }
  }
  function errorEvent() {
    $joinButton.disabled = true;
